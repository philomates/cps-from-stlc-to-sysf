...
Definition t_extend_wftenv_delta (D : env_type) (G : env_term (t_wft D))
 ...
 map (fun t => [obj t, t_wft_weaken D X (obj t) (pf t) okX]) G.


Lemma t_wft_open : forall D t (t' : typ_satisfying (t_wft D)),
  (forall X, X \notin dom D -> t_wft (D & X ~ star) (open_tt_var t X)) ->
...
Inductive t_typing (D : env_type) : env_term (t_wft D) ->
  ...
  | t_typing_app : forall G u1 (t : typ_satisfying (t_wft D)) u2 t1 t2,
      forall (pf1 : forall X, X # D -> t_wft (D & X ~ star) (open_tt_var t1 X)),
      forall (pf2 : forall X, X # D -> t_wft (D & X ~ star) (open_tt_var t2 X)),
      t_typing D G u1 [t_typ_arrow t1 t2,
                       t_wft_arrow (dom D) D t1 t2 pf1 pf2; t_wft D] ->
