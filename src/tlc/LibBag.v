...
(* ---------------------------------------------------------------------- *)
(** ** [forall x \inLN E, P x] notation *)

Notation "'forall_' x '\inLN' E ',' P" := 
  (forall x, x \inLN E -> P)
  (at level 200, x ident) : container_scope.
Notation "'forall_' x y '\inLN' E ',' P" := 
  (forall x y, x \inLN E -> y \inLN E -> P)
  (at level 200, x ident, y ident) : container_scope.
Notation "'forall_' x y z '\inLN' E ',' P" := 
  (forall x y z, x \inLN E -> y \inLN E -> z \inLN E -> P)
  (at level 200, x ident, y ident, z ident) : container_scope.

(* ---------------------------------------------------------------------- *)
(** ** [exists x \inLN E st P x] notation *)

Notation "'exists_' x '\inLN' E ',' P" := 
  (exists x, x \inLN E /\ P)
  (at level 200, x ident) : container_scope.
Notation "'exists_' x y '\inLN' E ',' P" := 
  (exists x, x \inLN E /\ y \inLN E /\ P)
  (at level 200, x ident, y ident) : container_scope.
Notation "'exists_' x y z '\inLN' E ',' P" := 
  (exists x, x \inLN E /\ y \inLN E /\ z \inLN E /\ P)
  (at level 200, x ident, y ident, z ident) : container_scope.
...
Class In_union_eq `{BagIn A T, BagUnion T} :=
  { in_union_eq : forall x (E F : T), x \inLN (E \u F) = (x \inLN E \/ x \inLN F) }.
Class In_union_l `{BagIn A T, BagUnion T} :=
  { in_union_l : forall x E F, x \inLN E -> x \inLN (E \u F) }.
Class In_union_r `{BagIn A T, BagUnion T} :=
  { in_union_r : forall x E F, x \inLN F -> x \inLN (E \u F) }.
Class In_union_inv `{BagIn A T, BagUnion T} :=
  { in_union_inv : forall x (E F : T), x \inLN (E \u F) -> (x \inLN E \/ x \inLN F) }.
...
Class In_inter_eq `{BagIn A T, BagInter T} :=
  { in_inter_eq : forall x E F, x \inLN (E \n F) = (x \inLN E /\ x \inLN F) }.
Class In_inter `{BagIn A T, BagInter T} :=
  { in_inter : forall x E F, x \inLN E -> x \inLN F -> x \inLN (E \n F) }.
Class In_inter_inv `{BagIn A T, BagInter T} :=
  { in_inter_inv : forall x E F, x \inLN (E \n F) -> x \inLN E /\ x \inLN F }.
...
Class In_remove_eq `{BagIn A T, BagRemove T T} :=
  { in_remove_eq : forall x (E F : T), x \inLN (E \- F) = (x \inLN E /\ x \notinLN F) }.

Implicit Arguments in_remove_eq [[A] [T] [H] [H0] [In_remove_eq]].

Class In_double_eq `{BagIn A T} :=
  { in_double_eq : forall E F, (forall x, x \inLN E = x \inLN F) -> E = F }.
Class In_double `{BagIn A T} :=
  { in_double : forall E F, (forall x, x \inLN E <-> x \inLN F) -> E = F }.
...
Class Incl_in `{BagIn A T, BagIncl T} :=
  { incl_in : forall x E F, E \c F -> x \inLN E -> x \inLN F}.
...
Class Single_incl `{BagSingle A T, BagIn A T, BagIncl T} :=
  { single_incl : forall x E, (\{x} \c E) = (x \inLN E) }.
