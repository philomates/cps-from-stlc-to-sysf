...
Notation "\set{ x | P }" := (@set_st _ (fun x => P))
  (at level 0, x ident, P at level 200).
Notation "\set{ x '\inLN' E | P }" := (@set_st _ (fun x => x \inLN E /\ P))
  (at level 0, x ident, P at level 200).

Notation "\set{= e | x '\inLN' E }" := 
  (@set_st _ (fun a => exists_ x \inLN E, a = e ))
  (at level 0, x ident, P at level 200).
Notation "\set{= e | x '\inLN' E , y ''\inLN' F }" := 
  (@set_st _ (fun a => exists_ x \inLN E, exists_ y \inLN F, a = e ))
  (at level 0, x ident, P at level 200).
Notation "\set{= e | x y '\inLN' E }" := 
  (@set_st _ (fun a => exists_ x y \inLN E, a = e ))
...
Notation "x \inLNdom E" := (x \inLN (dom E : set _)) 
...
Definition bigunion_impl A (E : set (set A)) : set A := 
  \set{ x | exists_ F \inLN E, x \inLN (F:set A) }.

Definition biguinter_impl A (E : set (set A)) : set A := 
  \set{ x | forall_ F \inLN E, x \inLN (F:set A) }.

(* ---------------------------------------------------------------------- *)
(** ** Image and pre-image *)

Definition image A B (f : A -> B) (E : set A) : set B :=
  \set{ y | exists_ x \inLN E, y = f x }.

Definition preimage A B (f : A -> B) (E : set B) : set A :=
  \set{ x | exists_ y \inLN E, y = f x }.
...
Ltac in_union_get :=
  match goal with H: ?x \inLN ?A |- ?x \inLN ?B =>
...
Hint Extern 3 (_ \inLN _ \u _) => in_union_get.
...
Hint Extern 3 (_ \inLN _) => in_union_extract.
...
Lemma empty_eq_single_inv_1 : forall x l1 l2,
  l1 = l2 -> x \notinLN l1 -> x \inLN l2 -> False.
Proof. intros. subst*. Qed.
Lemma empty_eq_single_inv_2 : forall x l1 l2,
  l1 = l2 -> x \notinLN l2 -> x \inLN l1 -> False.
Proof. intros. subst*. Qed.
...
Ltac in_union_meta :=
  match goal with 
  | |- _ \inLN \{_} => apply in_single_self
  | |- _ \inLN \{_} \u _ => apply in_union_l; apply in_single_self
  | |- _ \inLN _ \u _ => apply in_union_r; in_union_meta
  end.
...
Lemma set_in_union_inv : forall x l1 l2,
  x \inLN (l1 \u l2) -> x \inLN l1 \/ x \inLN l2.
...
Ltac set_in_inv_base H M :=
  match type of H with
  | _ \inLN \{} => false; apply (@set_in_empty_inv _ _ H)  
  | _ \inLN \{_} => 
