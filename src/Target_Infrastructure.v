...
Require Import LibLN Target_Definitions.
...
Fixpoint fv_tt (T : typ) {struct T} : vars :=
  match T with
  | typ_bool        => \{}
  | typ_pair T1 T2  => (fv_tt T1) \u (fv_tt T2)
  | typ_bvar J      => \{}
  | typ_fvar X      => \{X}
...
Fixpoint fv_te (e : trm) {struct e} : vars :=
  ...
  | trm_true      => \{}
  | trm_false     => \{}
  | trm_pair e1 e2 => (fv_te e1) \u (fv_te e2)
  | trm_abs T e1  => (fv_tt T) \u (fv_te e1)
  | trm_if v e1 e2 => (fv_te v) \u (fv_te e1) \u (fv_te e2)
  | trm_let_fst v e2 => (fv_te v) \u (fv_te e2)
  | trm_let_snd v e2 => (fv_te v) \u (fv_te e2)
  | trm_app e1 T e2 => (fv_te e1) \u (fv_tt T) \u (fv_te e2)
  end.
...
Fixpoint fv_ee (e : trm) {struct e} : vars :=
  ...
  | trm_true      => \{}
  | trm_false     => \{}
  | trm_pair e1 e2 => (fv_ee e1) \u (fv_ee e2)
  | trm_abs T e1  => (fv_ee e1)
  | trm_if v e1 e2 => (fv_ee v) \u (fv_ee e1) \u (fv_ee e2)
  | trm_let_fst v e1 => (fv_ee v) \u (fv_ee e1)
  | trm_let_snd v e1 => (fv_ee v) \u (fv_ee e1)
  | trm_app e1 T e2 => (fv_ee e1) \u (fv_ee e2)
  end.
...
Fixpoint subst_te (Z : var) (U : typ) (e : trm) {struct e} : trm :=
  ...
  | trm_pair e1 e2 => trm_pair (subst_te Z U e1) (subst_te Z U e2)
  | trm_abs T e1  => trm_abs  (subst_tt Z U T)  (subst_te Z U e1)
  | trm_if v e1 e2 => trm_if (subst_te Z U v) (subst_te Z U e1) (subst_te Z U e2)
  | trm_let_fst v e1 => trm_let_fst (subst_te Z U v) (subst_te Z U e1)
  | trm_let_snd v e1 => trm_let_snd (subst_te Z U v) (subst_te Z U e1)
  | trm_app e1 T e2 => trm_app (subst_te Z U e1) (subst_tt Z U T) (subst_te Z U e2)
  end.
...
Fixpoint subst_ee (z : var) (u : trm) (e : trm) {struct e} : trm :=
  ...
  | trm_pair e1 e2 => trm_pair (subst_ee z u e1) (subst_ee z u e2)
  | trm_abs T e1  => trm_abs T (subst_ee z u e1)
  | trm_if v e1 e2 => trm_if (subst_ee z u v) (subst_ee z u e1) (subst_ee z u e2)
  | trm_let_fst v e1 => trm_let_fst (subst_ee z u v) (subst_ee z u e1)
  | trm_let_snd v e1 => trm_let_snd (subst_ee z u v) (subst_ee z u e1)
  | trm_app e1 T e2 => trm_app (subst_ee z u e1) T (subst_ee z u e2)
  end.
...
Hint Resolve typing_var typing_app.
...
Lemma map_subst_tb_id : forall G Z P,
  okt G -> Z # G -> G = map (subst_tt Z P) G.
Proof.
